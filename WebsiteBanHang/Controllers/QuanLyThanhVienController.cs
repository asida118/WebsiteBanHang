    ...
    public class QuanLyThanhVienController : Controller
    {
        ...
        public ActionResult ChinhSua(ThanhVien tv)
        {
            if (ModelState.IsValid)
            {
                ...
                TryUpdateModel(thanhVien, new string[] { "MaThanhVien", "TaiKhoan", "MatKhau", "HoTen", "DiaChi", "Email",
        ...
        public ActionResult Xoa(int id)
        {
            ...
            {
                var ktTonTai = db.KhachHangs.Any(p => p.MaThanhVien == id);
                if (ktTonTai)
                    return View("BaoLoi", model: $"Lỗi xóa dữ liệu.<br>Lý do: đã có khách hàng phụ thuộc, vui lòng xóa khách hàng trước");
                else
